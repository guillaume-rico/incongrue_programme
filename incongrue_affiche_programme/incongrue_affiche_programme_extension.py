...
class changeInlineTextBalise(inkex.EffectExtension):
    ...
    def effect(self):
        ...
        for elem in self.listeEvenement :
            if elem["date"] != "" :
                layer.append(self.add_texte(elem["date"], 0, yDate + offsetY * numElem, "large", 9))
            if elem["heure"] != "" :
                layer.append(self.add_texte(elem["heure"],0,  yHeure + offsetY * numElem, "normal", 9))                    
            if len(elem["titre"]) <= 35 :
                layer.append(self.add_texte(elem["titre"], 185, yDate + offsetY * numElem, "large", 33))
            else :
                # On cherche quand séparer le texte
                for numCar in range (35, 0, -1) :
                    if elem["titre"][numCar] == " " :
                        break
                layer.append(self.add_texte(elem["titre"][0:numCar], 185, yDate + offsetY * numElem, "large", 33))
                layer.append(self.add_texte(elem["titre"][numCar:], 185, yDate + 25 + offsetY * numElem, "large", 33))
        ...
        for node in self.svg.xpath("//svg:text") :
            ...
            for item in node.xpath('./*') :
                for texteARemplacer in self.listeMotsCles :
                    if item.text != None and texteARemplacer in item.text :
        ...
        if self.options.couleur_fond == "f4900c" :
            ...
            fond = "#75924b"
            rayure = "#95bed5"
        self.svg.getElementById("path6398").style.set_color(fond, 'fill')
        self.svg.getElementById("path6402").style.set_color(rayure, 'fill')
        self.svg.getElementById("path6410").style.set_color(rayure, 'fill')
            
    def add_texte(self, dateelem, x , y, size, nbcarmax):
        ...
        decalageX = (nbcarmax - len(dateelem)) * multiplier + x
        my_text.transform = inkex.Transform('rotate(-3) translate(' + str(decalageX) + ', ' + str(y) + ')')
        return my_text
